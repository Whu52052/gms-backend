...
const DistributedConfig = {
  ...
  init() {
    ...
    // 从 URL 参数或全局配置读取次服务器地址
    // 支持: ?secondary=http://192.168.1.100:8765 或 window.__GMS_SECONDARY_URL__
    const secondaryParam = new URLSearchParams(window.location.search).get('secondary');
    if (secondaryParam) {
      const sec = this.servers.find(s => s.id === 'secondary-1');
      if (sec) { sec.url = secondaryParam; sec.enabled = true; }
    }
    if (window.__GMS_SECONDARY_URL__) {
      const sec = this.servers.find(s => s.id === 'secondary-1');
      if (sec) { sec.url = window.__GMS_SECONDARY_URL__; sec.enabled = true; }
    }
    // Fallback: 尝试从 localStorage 读取已保存的次服务器 URL
    const sec1 = this.servers.find(s => s.id === 'secondary-1');
    if (sec1 && !sec1.url) {
      const savedUrl = localStorage.getItem('gms_secondary_url');
      if (savedUrl) {
        sec1.url = savedUrl;
        sec1.enabled = true;
      }
    ...
    console.log('[Distributed] 服务器列表:', this.servers.filter(s => s.enabled && s.url).map(s => `${s.name}(${s.url})`).join(', '));
   ...
   * 便捷方法: 设置次服务器地址
   * 在部署页或管理后台调用
   * @param {string} url 次服务器地址，如 'http://192.168.1.101:8765'
   * @param {number} index 次服务器序号 (1 或 2)
   */
  setSecondaryUrl(url, index = 1) {
    const server = this.servers.find(s => s.id === `secondary-${index}`);
    if (server) {
      server.url = url;
      server.enabled = true;
      this.saveServerConfig();
      localStorage.setItem('gms_secondary_url', url);
      // 立即健康检查
      this._checkServerHealth(server);
      console.log(`[Distributed] 次服务器${index} URL 已设置: ${url}`);
    }
  },
